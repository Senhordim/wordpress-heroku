...
if ( ! function_exists('swm_register_menu')) {
	function swm_register_menu() {
		register_nav_menus(array(		
			...
			'top-bar-navigation' => __('Top Bar Navigation', 'swmtranslate'),		
...
if ( ! function_exists('swm_default_topmenu')) {
	function swm_default_topmenu() {		
		echo __('<ul><li class="menu-setting-msg">Set Top Menu from Wordpress Admin > Appearance > Menus > "Manage Locations" Box</li><li class="menu-setting-msg2">Add Top Menu icon from Wordpress Admin > Customizer > Main Navigation > Active Link Styling</li></ul>', 'swmtranslate');
	} // end function
} // end if

if ( ! function_exists('swm_default_footer_menu')) {
	function swm_default_footer_menu() {		
		echo __('<ul><li class="footer_menu-setting-msg">Set Footer Menu from Wordpress Admin > Appearance > Menus > "Manage Locations" Box</li></ul>', 'swmtranslate');
	} // end function
} // end if
				...
				<div class="swm_horizontal_menu filter_menu">
					<ul>
						...
						<?php if ($swm_excluce_pf_cat_tabs):
								wp_list_categories(array('title_li' => '', 'taxonomy' => 'portfolio-categories', 'exclude' => $swm_excluce_pf_cat_tabs, 'order' => 'asc', 'walker' => new Portfolio_Walker())); 
							else:
		...
		if ( $swm_post_format == 'quote' ) {			   			
			...
			if ( $swm_post_format == 'link' ) {
				if ( $blog_style == 'blog-style-grid' ) { 
					...
					echo '<div class="grid_date"><span><i class="fa fa-clock-o"></i>'.get_the_date('d F, Y').'</span><span><i class="fa fa-comment-o"></i><a href="' . get_permalink() . '">'.get_comments_number(get_the_ID()).' '.__('Comments', 'swmtranslate').'</a></span></div>';
				} else {
					echo '<h2>'.get_the_title().'</h2>';
				}
			} else {				
...
if (!function_exists('swm_page_title')) {
	function swm_page_title($title='') {
		...
		//event calendar pages
		if ( class_exists( 'TribeEvents' ) && !is_front_page() && !is_singular('page') ) {
